...
export default function Withdrawal() {
  ...
  useEffect(() => {
    ...
    const userData = JSON.parse(localStorage.getItem("userData")) || { _id: "user123" }
    setUser(userData)
  }, [])
  ...
  const fetchWithdrawalHistory = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setWithdrawalData(response.data.withdrawHistory)
      } else {
  ...
  }, [fetchWithdrawalHistory, refreshTrigger, user])
  ...
  const handleWithdrawl = async (e) => {
    ...
    const token = localStorage.getItem("authToken")
    try {
      const response = await axios.post(`${server}api/v1/payment/request/withdraw`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold tracking-tight">Withdrawal Management</h2>
        <button
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow">
        ...
        {isLoading ? (
          ...
          <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-md m-4">
            <p>
              No withdrawal history found. Use the &quot;Request Withdrawal&ldquo; button to make your first withdrawal request.
            </p>
          </div>
        ) : (
