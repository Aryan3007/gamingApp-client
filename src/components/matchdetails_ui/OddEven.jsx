...
const OddEven = ({ data }) => {
  const renderOddsBox = (odds, type) => {
      ...
      return (
        <div
          className={`w-full sm:w-12 md:w-16 h-8 ${
            type === "Odd" ? "bg-[#00b3ff36]" : "bg-[#ff7a7e42]"
    ...
    return (
      <button
        className={`w-full sm:w-12 md:w-16 h-8 ${
          type === "Odd" ? "bg-[#00B2FF] hover:bg-[#00A1E6]" : "bg-[#FF7A7F] hover:bg-[#FF6B6F]"
            ...
            <div className="flex flex-wrap sm:flex-nowrap justify-between items-center">
              ...
              {market.market.status === "SUSPENDED" ? (
                <div className="flex gap-2 w-full sm:w-auto">
                  <div className="w-1/2">{renderOddsBox(null, "Odd")}</div>
                  <div className="w-1/2">{renderOddsBox(null, "Even")}</div>
                </div>
              ) : (
                <div className="flex gap-2 w-full sm:w-auto">
                  <div className="w-1/2">{renderOddsBox(market.odds?.back[0], "Odd")}</div>
                  <div className="w-1/2">{renderOddsBox(market.odds?.lay[0], "Even")}</div>
                </div>
              )}
            </div>
          </div>
